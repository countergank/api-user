...
| AL-06 | Admin can manually unlock | POST /auth/admin/unlock/:userId guarded by admin role MUST reset `failedLoginAttempts` to 0 and clear `lockedUntil` |
...
**When** POST /auth/admin/unlock/:userId is called with the locked user's ID
...
**When** they attempt POST /auth/admin/unlock/:userId
...
### i18n Translation Keys (to be added)
...
| `src/auth/auth.controller.ts` | Add admin unlock endpoint |
